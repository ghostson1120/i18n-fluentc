import { defaults, defer, getCacheData, isMissingOption, saveCacheData } from './utils.js'
...
class Backend {
  ...
  read (language, namespace, callback) {
    ...
    const readFromService = new Promise((resolve) => {
      this.loadUrl({}, getContent(this.options.environmentId, language), (err, ret, info) => {
        ...
        saveCacheData(this.options.store, { lang: language, data: objectRet });

        resolve({err, data: objectRet})
      })
    });

    const readFromStore = new Promise(resolve => {
      if (!this.options.store) return;
      const cacheData = getCacheData(this.options.store);
      if (cacheData) {
        const data = cacheData.find(c => c.lang === language);
        if (data) {
          resolve({err: null, data: data.data });
        }
      }
    });

    Promise.race([readFromService, readFromStore])
      .then(({err, data}) => {
        callback(err, data);
      })
  }
